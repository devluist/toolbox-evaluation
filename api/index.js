...
app.get('/files/data', (req, res, next) => {
  ...
  if (fileName && fileName.length > 0) {
    https.request(API_FILES_URL + fileName, options, csvRes => { csvResponseHandler(csvRes, filesFetcher, res, next) }).end()
  }
      ...
      if (csvData.length > 1) {
        for (let j = 1; j < csvData.length; j++) {
          ...
          if (line.length > 3 && line[0] && line[1] && line[2] && line[3]) {
            ...
            const numb = line[2].length < 15 ? parseInt(line[2]) : BigInt(line[2])
